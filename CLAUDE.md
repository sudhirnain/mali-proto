...
- **Wired terminal buttons on every entry form.** Kicks: Done saves `{count} kicks, {min} min` with duration. Contractions: Stop toggles running state + counter; Done saves `{N} contractions, ~45s each`. Timer/Measurement/Event/Note all use SaveBar wired to useSaveEntry. No more dead Done buttons. `SaveBar` is sticky to the form bottom (Jonas round-2 s5 — Save always visible).
- **All timer categories use the retro start/end form** (Jonas round-2 s4). `TimerEntryForm` in `log/[category]/page.tsx` renders Start/End fields as the primary method for *every* `formKind: "timer"` category (sleep, sleep-mom, nursing, bottle, pumping, stroll, bathing) — historic data is always enterable — with a live-timer escape hatch and per-category extras (side / ml / milk-type / Daytime-Night). The old bare-stopwatch `TimerForm` is deleted; **don't reintroduce a stopwatch-only timer form.**
