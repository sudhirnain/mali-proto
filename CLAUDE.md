...
The shareable demo lands a stranger on `/feed` (root redirects there via `(prototype)/page.tsx`). The DemoNavigator widget — always visible top-right on desktop, compact pill on mobile — is the discovery surface. It has Phase / State / Adjust sections ("Jump to" and "In case you missed" both removed 2026-06-02 per Sudhir — the widget stays minimal; reviewers navigate in the app). No separate welcome flow. To demo the birth handoff manually: Pregnancy → /feed → flip phase to Parenting.
...
**The production APK palette IS the default since 2026-06-02 (Sudhir's call).** Extracted from mali-2.9.4.apk resources.arsc + webview CSS: coral = froly `#f08180` (their pink + link color), coral-dark = maliRed `#d14747`, coral-soft = mildPeach `#f2b8b8`, coral-softer = azalea `#fbdfdf`, teal = paradiso `#2c746d` (EY screens), teal-soft = green_light `#cedfdd`, cream = old-lace `#fdf6ee`; body text `#595959`; serif = Crimson Pro, Thai = Sarabun. Tokens marked "derived" in globals.css (coral-bright, teal-dark/-softer/-bright) have no production counterpart — computed from attested ones. Full extraction table: `mali-source/notes/apk-brand-colors.md` (gitignored). The Adjust panel's APK preset was removed — Reset now lands on these values. **Don't revert to the old warmer set** (`#e76f66`/`#3a9090`) — that was the pre-extraction guess. Known watch-item: `cat-growth #f4818a` is now nearly indistinguishable from primary froly — flagged, awaiting Sudhir's call on nudging growth deeper rose.
...
Categories keep their own colors (sleep = indigo, food = green, diaper = butter, etc.) — those are per-category, not per-phase. **Pregnancy-track categories alias the brand tokens** (declared as var() refs in BOTH the `@theme` and `:root` copies of globals.css): `cat-kicks` = `var(--color-coral-dark)`, `cat-kicks-soft` = coral-soft, `cat-contractions` = coral-dark, `cat-milestone` = coral-dark. **They alias coral-DARK (maliRed), not coral** — froly is an accent/link color and unreadable as an icon on a soft circle (~1.4:1; the 2026-06-02 audit fixed this). Same audit: `cat-growth` = `#e0566b` deeper rose (old `#f4818a` clashed with froly + weak on its soft). Don't re-inline aliased hexes; edit the brand token. The Adjust panel shows aliases as "= coral dark" chips, not inputs.

**Color-pair audit rules (2026-06-02, all screens verified):** tile surfaces (composer tiles, Moments `CategoryTile`, feed `QuickLogCard`/`MiniLogTile`, WelcomeCard rows) ALWAYS color via `tileColor(cat)` — never raw `cat.color` (that drift was Jonas's "icons don't match the backgrounds anymore"). Detail surfaces (log form wash + badge, category-detail header, entry-detail header, Timeline filter chips, entry rows) use raw `cat.color` (domain color) — intentional split, don't "fix" it. The "Right now" strip uses colored icons on white circles — its own surface treatment, also fine. Headless-Chrome screenshots clip ~50px on the right at phone widths — confirmed artifact (deployed build clips identically yet fits on Jonas's real phone); shoot at 600px width for full-frame verification, and after batch-editing globals.css confirm the dev server actually recompiled (grep the served CSS — Turbopack missed a scripted rewrite once).
...
- Phase-aware components use `usePhase()` from `@/lib/phase`. Always defer to that for any phase-dependent rendering. **`?phase=parenting` deep-links the phase** (applied in a post-mount effect; `lastChangeSource: "url"` suppresses the BirthHandoff celebration — only user switches celebrate). Used for shareable demo states and headless screenshot sweeps.
...
- **Status-bar bleed pattern**: `MobileFrame` does NOT add a universal `pt-11` for the fake iOS status bar. Each page owns its clearance: **tinted-top pages** (category detail / milestone / entry detail / log form) bleed to top:0 of the phone shell with internal `md:pt-[60px]` (44 status bar + 16 breathing). **White-top pages** (Timeline / Moments / Calendar / Feed / Profile / /log) add `md:pt-11` to their outer wrapper. `/feed` is a tinted-top page because of `FeedHeader`'s phase-tinted bg — the `md:pt-11` sits on the family-photo wrapper div inside FeedHeader (not the section or page wrapper), so the photo backdrop bleeds to top:0 behind the fake status bar.
...
- **Symmetric StatStrip side rings** — both phases now use the same SideStat treatment on both left and right (`border border-[var(--color-primary)]/40 bg-white/40`; `bg-white/85 backdrop-blur-sm` when over the family photo) so the side icons read as a pair flanking the center hero. Don't introduce stronger borders/fills on one side only.
- **Family-photo header (round-5, Jonas's mock C — settled after several iterations, don't rework):** user-set family photo behind the StatStrip ZONE only (never the whole header — arbitrary photos wreck the cards/text below). The photo stays **photographic**: no full-photo veil (dulled it), white-on-photo stat labels with text-shadow, a white top frost (the fake status bar has BLACK glyphs — a dark fade is wrong), a label darken that peaks `via-78%` then RELEASES (avoids a value cliff), and the pink/green melt confined to the bottom edge (`from-72%`). Camera badge = **dark glass** (`bg-black/30` + white glyph — white-on-primary reads as a third stat ring) at the photo's **top-right = final product position**; the DemoNavigator mobile pill lives top-LEFT specifically to keep that corner free. All fades are CSS overlays on whatever `mom.familyPhoto` holds — real user uploads inherit them; `FAMILY_PHOTOS` (5 verified Unsplash shots) + camera-cycling is demo-only. Cold mode: `familyPhoto: null` → flat tint + dark text (the white-text flip keys off the `onPhoto` prop).
- ~~DemoNavigator "In case you missed"~~ — REMOVED 2026-06-02 per Sudhir (along with "Jump to"); the widget is now Phase / State / Adjust only. Kept for the record: a one-click birth-handoff demo needs a ~350ms setTimeout between `setPhase("pregnancy")` and `setPhase("parenting")`, otherwise React batches the two calls and the BirthHandoff `useRef` transition detector never fires.
...
- **Article reader** (`/article/[slug]`, round-2 s10 "Show how we display an article") — mock articles in `src/lib/articles.ts` (8 seeded); every "Read more" is wired: feed `ArticleCard slug=`, category-detail insight (`articleForCategory`), milestone chart (`/article/milestones-pace`). Tinted hero block + white sheet, follows the tinted-top bleed pattern. No dead Read-more buttons — new ArticleCards must pass a slug or accept the dead-button fallback.
- **Milestone chart image-mode** (round-2 s12 "we don't have this graph as data, but just as pictures") — `src/lib/milestone-chart-images.ts` maps milestoneId → static chart PNG + axis calibration (x0/x1/monthsMax/yTop/yBottom as image fractions); ChartTab renders the image with OUR personalized "now" marker overlaid (baby age needs no curve data). One calibrated sample: `m-face` → `public/mali-art/charts/denver-m-face.png` (cropped from their production screenshot in the round-2 deck, their baked-in marker line healed out with PIL). Milestones without an image keep the live SVG chart. Per-image calibration is the cost of this approach — flag to Jonas if their PNGs don't share one layout.
- **Adjust panel** (DemoNavigator, "Adjust (for Mali team)" section — Jonas: "adjust the text and colours ourselves") — `TweakPanel.tsx`: designMode toggle for click-to-edit text (ephemeral, screenshot workflow) + color inputs for brand (coral/teal sets) and all category icon/tile-bg pairs. Overrides injected as `:root, [data-phase] { … !important }` style tag (out-ranks the parenting teal block; brand vars cascade into `--color-primary*` via var()), persisted in localStorage, applied on boot by `useStoredColorTweaks()` in MobileFrame (so saved colors also apply on mobile, where the panel is hidden). Desktop-only editing UI by design.
- **JourneyHero size comparison is the ANIMAL** (round-2 s3 + ladybug mock) — `baby.sizeAnimal {label, emoji}` in mock-baby.ts (emoji placeholder; Junporn owns final art; week→animal map is Mali's to finalize), falls back to watercolor + `sizeFruit` when unmapped. Bottom-row due-date link uses the **MilestoneHero treatment** (dotted underline + text "→", neutral-600) per s4 "Different style and arrow the same action… the one above [Milestones] looks correct" — keep the two hero cards' chrome identical (also: image square w-16 bg-white/70, title serif 21px on both).
...
- **Contractions = persistent tracker session** (Jonas round-2 s33 follow-up, 2026-06-02: "the timer seem to stop… I suggest we just put the timer in as a tracker, like with sleep"). Session state (in-flight `currentStart` + completed `{start,end}` events) lives in `ContractionSessionProvider` (`src/lib/contraction-session.tsx`, mounted in the prototype layout) — NOT component-local, so it survives navigation. The form registers `contractions` with `ActiveTimer` so the floating chip appears and links back; on each contraction stop the chip is re-baselined (`stop`+`start`) so its ticking number = **time since last contraction** (production Mali's page-top clock). The hero slot shows ONE clock, state-swapped (Sudhir 2026-06-02 — two ticking clocks compete): running → current-contraction duration (red) + frozen "after a X gap" context line; idle-with-history → ticking "Since last contraction" (the hospital-decision number) + static "Last contraction · 45s"; fresh → grey 00:00. **Don't reintroduce a second simultaneous ticking clock.** Done saves real stats (`N contractions, avg Xs, ~Y min apart` — gap < 90s renders as `~45s`, matching their Rate column) and finalizes an in-flight contraction. **Don't reintroduce:** the hardcoded `00:45` headline, `running` initialized `true`, or component-local session state — all three were exactly what Jonas couldn't understand.
